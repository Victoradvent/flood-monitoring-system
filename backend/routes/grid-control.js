  ...
  async (req, res) => {
    ...
    try {
      const equipmentRes = await pool.query(
        "SELECT * FROM grid_equipment WHERE id=$1",
        [id],
      );
        ...
        if (client.readyState === 1) {
          client.send(
            JSON.stringify({
              ...
              equipment,
              message: `Cutoff recommended for ${equipment.name}.`,
            }),
          );
        }
      });
      ...
      try {
        await logAudit(
          ...
          req.user.user_id,
          "RECOMMEND_CUTOFF",
          "Manual cutoff recommendation issued",
        );
      } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    res
      ...
      .json({
        error:
          "This deployment is recommendation-only; no physical power control is available.",
      });
  },
);
