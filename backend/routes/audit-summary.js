...
// Daily summary
router.get('/daily', authMiddleware, requireAnyRole(['admin', 'operator']), async (req, res) => {
  try {
...
// Weekly summary
router.get('/weekly', authMiddleware, requireAnyRole(['admin', 'operator']), async (req, res) => {
  try {
    const r = await pool.query(`
